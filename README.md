# Software Engineering Lab 1

## Requirements Engineering & UML Use-Case Modelling

**Problem Statement #04 — Campus & Academic Operations**

### Student Project Portfolio & Showcase App

This repository contains the deliverables for Software Engineering Lab 1.

## Deliverables

- [Requirements Table](Requirements/Requirements_Table.md)
  - 5 Functional Requirements (FR-001 to FR-005)
  - 2 Non-Functional Requirements (NFR-001 to NFR-002)
- UML Use-Case Diagram
- [Use-Case Flow Specification](Use-Case-Flow/Submit_Project_Use_Case.md)

## Actors

- Student Team
- Panel Judge
- Public Voter

## Core Use Case

**UC-01: Submit Project**
